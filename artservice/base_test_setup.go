...
type BaseSetupImpl struct {
    ...
    ChainID         string
    ChainCodeID     string
    SearchChainCodeID     string
    AppraisalChainCodeID string
    Initialized     bool
...
// InstallAndInstantiateArtChainCC ..
func (setup *BaseSetupImpl) InstallAndInstantiateArtChainCC(codepath string, version string) error {

    chainCodePath := codepath
...
// InstallAndInstantiateSearchCC ..
func (setup *BaseSetupImpl) InstallAndInstantiateSearchCC(codepath string, version string) error {

    chainCodePath := codepath
    chainCodeVersion := version

    if setup.SearchChainCodeID == "" {
        setup.SearchChainCodeID = fcUtil.GenerateRandomID()
    }

    if err := setup.InstallCC(setup.SearchChainCodeID, chainCodePath, chainCodeVersion, nil); err != nil {
        return err
    }
    ...
    return setup.InstantiateCC(setup.SearchChainCodeID, setup.ChainID, chainCodePath, chainCodeVersion, args)
}

// InstallAndInstantiateAppraisalCC ..
func (setup *BaseSetupImpl) InstallAndInstantiateAppraisalCC(codepath string, version string) error {

    chainCodePath := codepath
    chainCodeVersion := version

    if setup.AppraisalChainCodeID == "" {
        setup.AppraisalChainCodeID = fcUtil.GenerateRandomID()
    }

    if err := setup.InstallCC(setup.AppraisalChainCodeID, chainCodePath, chainCodeVersion, nil); err != nil {
        return err
    }
    ...
    return setup.InstantiateCC(setup.AppraisalChainCodeID, setup.ChainID, chainCodePath, chainCodeVersion, args)
}
...
func (setup *BaseSetupImpl) InvokeSearch(args []string) (string, error) {

    transientDataMap := make(map[string][]byte)
    transientDataMap["result"] = []byte("Transient data in move funds...")

    transactionProposalResponse, txID, err := fcUtil.CreateAndSendTransactionProposal(setup.Chain, setup.SearchChainCodeID, setup.ChainID, args, []fabricClient.Peer{setup.Chain.GetPrimaryPeer()}, transientDataMap)
    if err != nil {
        return "", fmt.Errorf("CreateAndSendTransactionProposal return error: %v", err)
    }
    ...
    return string(transactionProposalResponse[0].GetResponsePayload()), nil
}

func (setup *BaseSetupImpl) InvokeAppraisal(args []string) (string, error) {

    transientDataMap := make(map[string][]byte)
    transientDataMap["result"] = []byte("Transient data in move funds...")

    transactionProposalResponse, txID, err := fcUtil.CreateAndSendTransactionProposal(setup.Chain, setup.AppraisalChainCodeID, setup.ChainID, args, []fabricClient.Peer{setup.Chain.GetPrimaryPeer()}, transientDataMap)
    if err != nil {
        return "", fmt.Errorf("CreateAndSendTransactionProposal return error: %v", err)
    }
